...
#                           site_configs_path):
#     """Generate one PPTX resident report per site (keyed by SiteID).
...
#     Resident address/name/ZIP are PII and never appear in generated
#     reports — the cover letter says "Resident at {SiteID}" and the
#     output filename is "Resident_Report_{SiteID}.pptx".

#     Returns (report_count, list_of (site_id, filepath) tuples).
#     """
#     df = pd.read_csv(master_csv_path)
#     df = df[df['SampleID'].notna() & (df['SampleID'] != "")]
#     df['LeadPPM_Clean'] = df['LeadPPM'].apply(resolve_lod)

#     # Load site configs — one report per site_id
#     site_configs = {}
#     if os.path.exists(site_configs_path):
#         with open(site_configs_path, 'r') as f:
#             raw = json.load(f)
#         site_configs = {s["site_id"]: s for s in raw}
...
#             st.info(
#                 f"⏭️  Skipped '{site_id}' — no real XRF data matched yet."
#             )
#             continue
...
#         # underscores), so no sanitization needed.
#         map_image_path = os.path.join(maps_dir, f"map_{site_id}.png")
#         try:
#             render_static_png(
...
#                         # We deliberately do NOT fill "Address of Resident"
#                         # anymore (it's PII). The placeholder paragraph in
#                         # the template should be removed by the maintainer
#                         # of Resident_Report_Template.pptx; if the literal
#                         # text is still present we blank it out.
...
#                             # Strip the placeholder rather than fill it.
#                             run.text = txt.replace(
#                                 "Address of Resident", ""
#                             ).rstrip()
#                             txt = run.text
...
#                         )
#                     except Exception as e:
#                         st.warning(f"Map insert failed (slide 3) for {site_id}: {e}")
...
#                         )
#                     except Exception as e:
#                         st.warning(f"Map insert failed (slide 4) for {site_id}: {e}")

#         # ── 5. Save ──
#         output_file = os.path.join(
#             output_dir, f"Resident_Report_{site_id}.pptx"
#         )
...
#                 "🔍 Search reports by SiteID",
#                 placeholder="e.g. 2025-10-18, 2025-06-24...",
#                 key="report_search",
#             )
...
Updates from original:
  1. Imports NYSH colors & rotation helpers from groundsense_config.py
  2. Map image generation goes through `map_renderer.render_static_png()`
     — the same function site_builder.py uses for its PNG export.
  3. Honors `rotation_deg` saved in site_configs.json (via map_renderer).
  4. **Report assembly delegated to `src/report_generator.py`** so the
     PPTX-building logic stays out of this Streamlit shell and is
     reusable from cron jobs, tests, or the CLI.
  5. Data policy: only real XRF data is used in exported maps. Cells
     without matching Master Data render as gray "No Data".  Sites
     with no matching real readings are skipped, not faked.

Template slide layout (1-indexed page numbers; 0-indexed slide_idx):

  Page 1 / slide 0  Cover letter
      * "Address of Resident" → SiteID
      * "Name of Resident"    → SiteID
      * "Date"                → today
  Page 2 / slide 1  Sample collection method (no edits)
  Page 3 / slide 2  Soil Report Summary
      * Inserts a basemap+numbers screenshot of the Leaflet HTML map.
        Falls back to the white-theme static PNG when the screenshot
        can't be produced (offline / no browser).
  Page 4 / slide 3  Detailed Results
      * Fills Backyard / Front yard rows with real PPM averages.
        Missing yard → "-".
      * Inserts the WHITE-background static PNG (light_theme=True).
  Pages 5+ left untouched.

The user-facing UI (file uploader, KPIs, downloads, summary cards)
...
# Inject both the script's own directory AND src/ into sys.path so that
# `groundsense_config`, `map_renderer`, and `report_generator` can be
# imported regardless of whether they live in src/ or at the repo root.
_HERE = os.path.dirname(os.path.abspath(__file__))
for _p in (_HERE, os.path.join(_HERE, "src")):
    if _p not in sys.path:
        sys.path.insert(0, _p)

# Re-export a few helpers other modules / tests may still import from us.
from groundsense_config import (  # noqa: F401  (re-exported for callers)
    get_nysh_category,
    NYSH_TIERS,
    NYSH_COLORS,
    calculate_coordinate,
...
from map_renderer import render_static_png  # noqa: F401  (re-exported)

# All the heavy lifting for PPTX assembly lives in report_generator.
# We re-expose the zone helpers under their historical names so any
# existing import sites (notebooks, tests) keep working.
from report_generator import (
    compute_zone_averages,         # noqa: F401
    format_zone_ppm,               # noqa: F401
    generate_all_reports as _generate_all_reports,
)
...
# ═══════════════════════════════════════════════
#  REPORT GENERATION  (thin wrapper kept for backwards compat)
# ═══════════════════════════════════════════════
def generate_pptx_reports(master_csv_path, template_path, output_dir,
                          site_configs_path):
    """Generate one PPTX resident report per site_id.

    Thin wrapper around ``report_generator.generate_all_reports`` that
    routes log lines through ``st.info`` / ``st.warning`` so the
    Streamlit pipeline UI shows progress.

    Returns ``(report_count, [(site_id, filepath), ...])``.
    """
    def _log(msg: str) -> None:
        # Route the framework-free module's messages to Streamlit so
        # the user sees per-site status (skipped / generated / fell
        # back to static map).
        text = str(msg)
        if text.startswith("⏭️"):       # ⏭ = skip-forward
            st.info(text)
    ...
    return _generate_all_reports(
        master_csv_path=master_csv_path,
        template_path=template_path,
        output_dir=output_dir,
        site_configs_path=site_configs_path,
        log=_log,
    )
