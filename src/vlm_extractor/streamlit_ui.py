...
def _apply_to_session_state(
    ...
    for cid, cell in cells.items():
        try:
            ...
            # SampleID pattern: use the SKETCH label (what was written on
            # the page) so it matches XRF sample filenames like
            # "...Back_3F_..." rather than the internal "Back_F1_".
            sketch_lbl = cell.get("sketch_label") or cid
            _set(f"pat_{cid}_{yard_key}", f"{yard_choice_titlecase}_{sketch_lbl}_")
...
def _render_review_panel(
    ...
    # ── Refine shapes (drag-corner editor for any cell) ──
    _render_refine_shapes_section(yard_key, extraction)

    # ── Diagnostic: shape extraction summary (helps debug shape misses) ──
    shape_breakdown = {}
    for cid, cell in cells.items():
        kind = cell.get("shape_kind", "rect")
        shape_breakdown.setdefault(kind, []).append((cid, cell.get("sketch_label") or cid))
    non_rect_count = sum(len(v) for k, v in shape_breakdown.items() if k != "rect")
    rect_count = len(shape_breakdown.get("rect", []))

    with st.expander(
        f"🔬 Shape detection summary — "
        f"{non_rect_count} non-rect / {rect_count} rect "
        f"({'✅ shapes found' if non_rect_count else '⚠️  all cells rect'})",
        expanded=(non_rect_count == 0),
    ):
        for kind in ("rect", "notch", "angle", "custom"):
            entries = shape_breakdown.get(kind, [])
            if not entries:
                continue
            badge = {"rect": "▭", "notch": "◣", "angle": "◢", "custom": "⬢"}.get(kind, "?")
            labels = ", ".join(sl for _, sl in sorted(entries))
            st.markdown(f"{badge} **{kind}** ({len(entries)} cells): {labels}")
        if non_rect_count == 0:
            st.warning(
                "**All cells were extracted as plain rectangles.** "
                "If the sketch actually has angled or notched cells, the "
                "VLM missed them. Try:\n"
                "- Adding more explicit hints in ⚙️ Extraction settings "
                "(e.g. 'cell 3F has an angled top edge — slanted down by "
                "about 6 feet on the right side')\n"
                "- Switching to a different model in settings\n"
                "- Manually editing the shape in section ⑦ after accepting"
            )

    # ── Diagnostic: raw VLM response (for debugging extraction quality) ──
    raw_response = st.session_state.get(_ss_key(yard_key, "raw_response"), "")
    if raw_response:
        with st.expander("🔍 Raw VLM response (for debugging)", expanded=False):
            st.caption(
                "This is the exact JSON Claude/Gemini returned. Use it to "
                "verify: (1) did the VLM detect the shape but my code drop "
                "it? — look at the `shape_kind` field for each cell. (2) "
                "did the VLM mention the shape in its `notes` even if it "
                "set `shape_kind: rect`?"
            )
            # Show parsed JSON first (prettier), fall back to raw if parse fails
            try:
                import json as _json
                parsed = _json.loads(raw_response) if isinstance(raw_response, str) else raw_response
                st.json(parsed)
            except (ValueError, TypeError):
                st.code(raw_response[:4000], language="json")
...
def _render_extraction_table(extraction: dict[str, Any]) -> None:
    ...
    for row in rows:
        ...
        for c in range(1, max_cols + 1):
            cid = f"{row}{c}"   # row-first: A1, F3, etc.
            if c <= n_in_row and cid in cells:
                ...
                # Show the sketch label (what the user wrote on the page)
                # prominently; show the internal ID smaller below in muted
                # text so the user can map between them mentally.
                sketch_lbl = cell.get("sketch_label") or cid
                internal_note = ""
                if sketch_lbl != cid:
                    internal_note = (
                        f"<div style='font-size:0.7em;color:#999;'>"
                        f"internal: {cid}</div>"
                    )
                html.append(
                    ...
                    f"<div style='font-weight:600;'>{sketch_lbl}{walkway_badge}{gap_r_badge}</div>"
...
# ════════════════════════════════════════════════════════════════
#  Refine shapes — drag-corner editor wiring
# ════════════════════════════════════════════════════════════════

def _render_refine_shapes_section(yard_key: str, extraction: dict[str, Any]) -> None:
    """Render the 'Refine shapes' section: a list of cells with an
    Edit button per cell, and the active editor below.

    The editor is rendered inline (not in a popup) for the cell whose
    Edit button was most recently clicked, tracked in session state.
    Clicking Save inside the editor writes the new shape back into the
    extraction dict (also stored in session state) and clears the
    active-cell flag.
    """
    from .shape_editor import render_shape_editor  # lazy import

    cells = extraction.get("cells") or {}
    if not cells:
        return

    active_key = _ss_key(yard_key, "shape_editor_active")
    active_cell = st.session_state.get(active_key)

    with st.expander("✏️ Refine cell shapes (drag corners to adjust)", expanded=False):
        st.caption(
            "Click **Edit** beside any cell to open a drag-corner editor. "
            "Drag any of the four blue corners to reshape the cell — "
            "corners snap to the cell's edges so rectangles, single-edge "
            "angles, and corner notches stay clean. Save when done."
        )

        # ── Per-cell edit buttons, grouped by row ──
        rows_in_extraction = extraction.get("rows", [])
        for row in rows_in_extraction:
            row_cells = sorted(
                [(cid, c) for cid, c in cells.items() if c.get("row") == row],
                key=lambda kv: kv[1].get("col", 0),
            )
            if not row_cells:
                continue
            st.markdown(f"**Row {row}**")
            cols = st.columns(min(len(row_cells), 4))
            for i, (cid, cell) in enumerate(row_cells):
                with cols[i % len(cols)]:
                    sketch_lbl = cell.get("sketch_label") or cid
                    shape = cell.get("shape_kind", "rect")
                    shape_badge = "" if shape == "rect" else f"  · {shape}"
                    btn_label = f"✏️ {sketch_lbl}{shape_badge}"
                    btn_key = _ss_key(yard_key, f"edit_{cid}")
                    is_active = (active_cell == cid)
                    if st.button(
                        btn_label,
                        key=btn_key,
                        ...
                        # Toggle: clicking the active cell closes the editor.
                        if is_active:
                            st.session_state.pop(active_key, None)
                        else:
                            st.session_state[active_key] = cid
                        st.rerun()

        # ── Active editor ──
        if active_cell and active_cell in cells:
            st.markdown("---")
            cell = cells[active_cell]
            sketch_lbl = cell.get("sketch_label") or active_cell
            st.markdown(
                f"### Editing  **{sketch_lbl}**  _(internal: {active_cell})_"
            )

            editor_key = _ss_key(yard_key, f"editor_{active_cell}")
            new_shape = render_shape_editor(active_cell, cell, key=editor_key)

            # Cancel button — clears the active editor without saving.
            col_cancel, _ = st.columns([1, 3])
            with col_cancel:
                if st.button(
                    "✖ Cancel (discard changes)",
                    key=_ss_key(yard_key, f"cancel_{active_cell}"),
                    use_container_width=True,
                ):
                    st.session_state.pop(active_key, None)
                    st.rerun()

            # When the JS widget reports a save, commit it into the
            # extraction stored in session state.
            if new_shape is not None:
                _commit_shape_edit(yard_key, active_cell, new_shape)
                st.success(
                    f"✅ Saved new shape for **{sketch_lbl}**: "
                    f"`{new_shape['shape_kind']}`"
                )
                st.session_state.pop(active_key, None)
                st.rerun()


def _commit_shape_edit(
    yard_key: str,
    cell_id: str,
    new_shape: dict[str, Any],
) -> None:
    """Write a shape edit back into the session-state extraction."""
    extraction = st.session_state.get(_ss_key(yard_key, "extraction"))
    if not extraction:
        return
    cells = extraction.get("cells") or {}
    ...
    if not cell:
        return
    cell["shape_kind"]    = new_shape["shape_kind"]
    cell["shape_params"]  = new_shape.get("shape_params") or {}
    cell["local_polygon"] = new_shape.get("local_polygon")
    # Mark the cell as human-verified so the review highlights drop.
    cell["confidence"] = "high"
    cell.setdefault("notes", "")
    if cell["notes"]:
        cell["notes"] += " · "
    cell["notes"] += "shape refined via drag editor"
    # Persist back.
    st.session_state[_ss_key(yard_key, "extraction")] = extraction