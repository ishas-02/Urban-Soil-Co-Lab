...
#                         "default": "",
#                         "description": (
#                             "The original label as written on the sketch "
#                             "(e.g. '3F' for an extension cell internally "
#                             "named F1). Used for user-facing display only "
#                             "so the review panel matches the page."
#                         ),
#                     },
#                 },
#             },
...
#             if not isinstance(cell, dict):
#                 continue
#             # ID format <row><col>, e.g. "A1", "F3"
#             m = re.match(r"^([A-Z])(\d+)$", cid)
#             if not m:
#                 problems.append(
#                     f"cell key '{cid}' must match `<row><col>` "
#                     f"(e.g. 'A1', 'F3') — letter MUST come first, uppercase"
#                 )
#                 continue
#             id_row, id_col = m.group(1), int(m.group(2))
#             cell_col = cell.get("col")
#             cell_row = cell.get("row")
#             if cell_col != id_col:
#                 problems.append(
...
#         c.setdefault("confidence", "medium")
#         c.setdefault("notes", "")
#         # If the VLM didn't provide a sketch_label (older responses or
#         # forgetful prompts), fall back to the internal cell ID flipped
#         # to column-first — that's how field workers usually write it.
#         if not c.get("sketch_label"):
#             import re as _re
#             m = _re.match(r"^([A-Z])(\d+)$", cid)
#             c["sketch_label"] = f"{m.group(2)}{m.group(1)}" if m else cid
#         cells_out[cid] = c
#     out["cells"] = cells_out
#     return out
...
#         default_pat = f"{yard_choice}_{sketch_lbl}_"
#         cell_data[cid] = {
#             "width":         float(cell["width"]),
#             "height":        float(cell["height"]),
#             "col":           int(cell["col"]),
...
def normalize_extraction(data: dict[str, Any]) -> dict[str, Any]:
    ...
    for cid, cell in (out.get("cells") or {}).items():
        ...
        c.setdefault("confidence", "medium")
        c.setdefault("notes", "")
        # If the VLM didn't provide a sketch_label, build it from the
        # cell's row + col in column-first form (how field workers
        # typically write it on the page: "3F", "4E", etc.).
        if not c.get("sketch_label"):
            row = c.get("row")
            col = c.get("col")
            if isinstance(row, str) and isinstance(col, int):
                c["sketch_label"] = f"{col}{row}"
            else:
                c["sketch_label"] = cid
        cells_out[cid] = c
    out["cells"] = cells_out
