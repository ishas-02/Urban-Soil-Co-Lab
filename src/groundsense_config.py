...
def load_site_configs(config_path=None):
    """Load site_configs.json. Returns a dict keyed by site_id.

    SiteID is the canonical identifier across the pipeline. Resident
    address/city/ZIP are PII and never live in site_configs.json — they
    belong only in the gitignored local PII file (see dashboard._resolve_pii_path).
    """
