...
# def load_existing_config_for_address(address, config_path):
#     """If this address already has a saved config, return its current offset/rotation."""
...
#     # ── Preserve existing rotation if this address was saved before ──
#     base_dir = os.path.dirname(os.path.abspath(__file__))
#     config_path = os.path.join(base_dir, "..", "data", "site_configs", "site_configs.json")
#     existing = load_existing_config_for_address(address, config_path)
...
#         found = False
#         for i, s in enumerate(existing):
#             if s.get("address") == updated_config["address"]:
...
#         st.success(
#             f"✅ **Position saved** for {updated_config['address']}.  \n"
...
def load_master_data():
    """Load the latest Master_Data_v*.csv for looking up real Lead PPM
...
"""
    ...
    safe_name = (
        config["address"].replace(" ", "_").replace("/", "_").replace(",", "")
    )
