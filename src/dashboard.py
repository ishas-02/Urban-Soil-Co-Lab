...
# # # #  Surfaces the per-row status produced by key_reconciler.py:
# # # #    - red    → discrepancy (chem & site disagree on LeadPPM)
# # # #    - yellow → pending     (only one side has the row yet)
# # # #    - (no color) → match
# # # #  Shown in an expander so it doesn't crowd the dashboard when clean.
# # # # ═══════════════════════════════════════════════
# # # def _render_reconciliation_panel(master_df: pd.DataFrame) -> None:
# # #     if master_df is None or master_df.empty or "status" not in master_df.columns:
# # #         return

# # #     counts = master_df["status"].value_counts()
# # #     n_disc = int(counts.get("discrepancy", 0))
# # #     n_pend = int(counts.get("pending", 0))
# # #     n_match = int(counts.get("match", 0))

# # #     if n_disc == 0 and n_pend == 0:
# # #         # All rows matched — show a quiet confirmation, no expander.
# # #         st.success(
# # #             f"✅ Reconciliation: all {n_match} rows match between "
# # #             f"chem data and site lab data."
# # #         )
# # #         return

# # #     header = f"🔁 Reconciliation: {n_disc} discrepancies · {n_pend} pending · {n_match} matched"
# # #     expanded_default = n_disc > 0  # auto-open only if there are real problems
...
# # #         flagged = master_df[master_df["status"].isin(["discrepancy", "pending"])].copy()
# # #         if flagged.empty:
# # #             return

# # #         display_cols = [c for c in ["SampleID", "XRFID", "LeadPPM", "LeadAvg", "Site_ID", "status"]
# # #                         if c in flagged.columns]
# # #         flagged = flagged[display_cols]

# # #         def _row_style(row: pd.Series) -> list[str]:
# # #             status = row.get("status", "")
# # #             if status == "discrepancy":
# # #                 # Soft red so dark text stays readable
...
# # #         st.dataframe(
# # #             flagged.style.apply(_row_style, axis=1),
# # #             use_container_width=True,
# # #             hide_index=True,
# # #         )


# # # _render_reconciliation_panel(master_df)
...
# # def _sample_yard(sample_id):
# #     """Classify a SampleID into 'front' or 'back' (defaults to 'back').

# #     Per project spec: SampleIDs without 'Front' or 'Back' (case-
# #     insensitive) default to backyard. Legacy data without yard markers
# #     in the name will only ever color back-yard blocks, never front.
# #     """
...
# # def match_sample_to_master(patterns, master_df, block_yard=None):
# #     """Find avg LeadPPM_Clean for SampleIDs matching any of the patterns.

# #     If `block_yard` is 'front' or 'back', only SampleIDs whose yard
# #     classification matches are considered. None preserves legacy
# #     behavior (no yard filter).
# #     """
# #     for pattern in patterns:
# #         matches = master_df[
# #             master_df['SampleID'].str.contains(pattern, case=False, na=False)
...
# #     """Look up anchor + rotation for a yard. Falls back to top-level if absent.
...
# # def _block_yard(dims):
# #     """Extract a block's yard key ('front'/'back'/None) from its dims."""
# #     yk = dims.get("yard")
# #     if yk:
# #         return yk
...
# #     Supports both legacy single-yard configs and the new multi-yard
# #     schema (`yards: {front, back}`). For multi-yard configs, each
# #     block is projected from its own yard's anchor + rotation, and a
# #     separate marker is placed for each yard's anchor.
# #     """
# #     # Legacy fields — used as the map center reference and as fallback
# #     # for any block that doesn't declare a yard.
# #     legacy_anchor = site_config.get("anchor", {"lat": 0, "lon": 0})
# #     legacy_rotation = site_config.get("rotation_deg", 0) or 0
# #     defaults = site_config.get("map_defaults", {})

# #     # For map centering: if multi-yard, use midpoint of all yard anchors.
# #     yards_block = site_config.get("yards") or {}
# #     if yards_block:
# #         anchor_pts = []
# #         for yk, y in yards_block.items():
# #             if y and y.get("anchor"):
# #                 anchor_pts.append((y["anchor"]["lat"], y["anchor"]["lon"]))
# #         if anchor_pts:
# #             ref_lat = sum(p[0] for p in anchor_pts) / len(anchor_pts)
# #             ref_lon = sum(p[1] for p in anchor_pts) / len(anchor_pts)
# #         else:
# #             ref_lat, ref_lon = legacy_anchor["lat"], legacy_anchor["lon"]
# #     else:
# #         ref_lat, ref_lon = legacy_anchor["lat"], legacy_anchor["lon"]
...
# #     # Place anchor markers — one per yard if multi-yard, else single legacy.
# #     if yards_block:
# #         for yk, y in yards_block.items():
# #             if not y or not y.get("anchor"):
# #                 continue
# #             ya = y["anchor"]
# #             marker_color = "orange" if yk == "front" else "red"
# #             folium.Marker(
# #                 location=[ya["lat"], ya["lon"]],
# #                 tooltip="<b>{}</b>".format(
# #                     ya.get("marker_label", f"{yk.capitalize()} Anchor")
# #                 ),
# #                 icon=folium.Icon(color=marker_color, icon='home')
# #             ).add_to(m)
# #     else:
# #         folium.Marker(
# #             location=[legacy_anchor["lat"], legacy_anchor["lon"]],
# #             tooltip="<b>{}</b>".format(legacy_anchor.get('marker_label', 'Anchor Point')),
# #             icon=folium.Icon(color='red', icon='home')
# #         ).add_to(m)
...
# #         # Resolve THIS block's anchor + rotation from its yard.
# #         block_yard = _block_yard(dims)
# #         b_anchor, b_rotation = _resolve_yard_anchor_and_rotation(
...
# #         corner_latlon = [
# #             calculate_coordinate_rotated(
# #                 b_anchor["lat"], b_anchor["lon"], cy, cx, b_rotation
# #             )
# #             for cx, cy in corners_ft
# #         ]
...
# #         # Display the human-readable cell_id if present, else the dict key.
# #         display_id = dims.get("cell_id", block_id)
...
# #         </div>
# #         """.format(
# #             block_id=display_id, color=color_hex, ppm=ppm,
# #             label=label, w=width_ft, h=height_ft, src=data_source
# #         )
...
# #         stats["blocks"].append({
# #             "id": display_id, "ppm": ppm, "label": label,
# #             "source": data_source, "zone": dims.get("zone", "")
# #         })
...
# #         pt_yard = _block_yard(pt)
# #         p_anchor, p_rotation = _resolve_yard_anchor_and_rotation(
# #             site_config, pt_yard
# #         )
# #         pt_lat, pt_lon = calculate_coordinate_rotated(
# #             p_anchor["lat"], p_anchor["lon"], oy, ox, p_rotation
# #         )
 
# #         patterns = pt.get("sample_id_patterns", [])
# #         # Auxiliary points: don't yard-filter unless explicitly tagged.
# #         real_ppm = match_sample_to_master(patterns, site_df,
# #                                            block_yard=pt_yard)
...
# #     # Stash rotation in stats for diagnostic display. For multi-yard
# #     # configs this is the legacy mirror rotation (front yard's).
# #     stats["rotation_deg"] = legacy_rotation
...
def generate_site_map(site_config, master_df):
    ...
    for block_id, dims in grid.items():
        ...
        stats["blocks"].append({
            "id": display_id, "ppm": ppm, "label": label,
            "source": data_source, "zone": dims.get("zone", ""),
            "yard": block_yard, "has_real_data": real_ppm is not None,
        })
    ...
    for pt_id, pt in points.items():
        ...
        stats["blocks"].append({
            "id": pt_id, "ppm": ppm or 0, "label": label,
            "source": data_source, "zone": pt.get("zone", ""),
            "yard": pt_yard, "has_real_data": real_ppm is not None,
        })
...
# Reconciliation panel hidden from the dashboard by request.
# The function above still works and `master_df["status"]` is still
# populated by key_reconciler — flip this flag to re-show the panel
# (discrepancy / pending / matched counts + flagged-row table).
_SHOW_RECONCILIATION_PANEL = False
if _SHOW_RECONCILIATION_PANEL:
    _render_reconciliation_panel(master_df)
...
if not master_df.empty and site_configs:
    ...
    if selected_site in site_configs:
        ...
        # ── Lead-level summary stats — split by yard if both present ──
        # Compute from real (lab-measured) values only; placeholder/mock
        # ppm values would skew the average and make highs/lows meaningless.
        real_blocks = [b for b in stats["blocks"]
                       if b.get("has_real_data") and b.get("ppm") is not None]

        def _ppm_stats_for(blocks_subset):
            ppms = [float(b["ppm"]) for b in blocks_subset
                    if b.get("ppm") is not None]
            if not ppms:
                return None
            return {
                "high": max(ppms),
        ...
        def _render_yard_stats(label_text, s):
            if s is None:
                st.caption(f"_{label_text}: no XRF readings yet._")
                return
            c1, c2, c3 = st.columns(3)
            c1.metric(
                f"{label_text} · Highest",
                f"{s['high']:.0f} ppm",
                help=f"Maximum lead reading among {s['n']} measured cell(s).",
            )
            c2.metric(
                f"{label_text} · Lowest",
                f"{s['low']:.0f} ppm",
                help=f"Minimum lead reading among {s['n']} measured cell(s).",
            )
            c3.metric(
                f"{label_text} · Average",
                f"{s['avg']:.0f} ppm",
                help=f"Mean lead reading across {s['n']} measured cell(s).",
            )

        # Yards present in this site (any block tagged front or back).
        yards_present = {b.get("yard") for b in stats["blocks"]
                         if b.get("yard") in ("front", "back")}

        if "front" in yards_present and "back" in yards_present:
            front_stats = _ppm_stats_for(
                [b for b in real_blocks if b.get("yard") == "front"]
            )
            back_stats = _ppm_stats_for(
                [b for b in real_blocks if b.get("yard") == "back"]
            )
            _render_yard_stats("Front Yard", front_stats)
            st.markdown("")
            _render_yard_stats("Back Yard", back_stats)
        else:
            # Single yard, or untagged blocks — one row of metrics.
            _render_yard_stats("Lead Level", _ppm_stats_for(real_blocks))
