...
def _resolve_pii_path():
    """Return absolute path to the local-only PII file, or None if unset/missing.
 
    PII (resident name, full street address, ZIP) used to live inside the
    field CSV in `data/site_databases/`. To avoid ever committing PII to the
    repo, that data has been split out to a user-configured local-only file:
 
      * Primary: `$SOIL_COLAB_PII_PATH` environment variable (absolute path).
      * Fallback: `~/.soil_colab/residents_private.csv`.
 
    The expected file is a plain single-header CSV with at minimum
    `SiteID` and `Address` columns. Other columns (`FirstName`, `LastName`,
    `City`, `ZipCode`) are optional and currently unused by this loader.
    """
    explicit = os.environ.get("SOIL_COLAB_PII_PATH")
    if explicit:
        return os.path.expanduser(explicit)
    fallback = os.path.expanduser("~/.soil_colab/residents_private.csv")
    return fallback if os.path.exists(fallback) else None
 
 
def _load_site_address_mapping():
    """Build SampleID -> Address mapping using two private, repo-free sources.
 
    Strategy:
      1. Read the local-only PII file (SiteID -> Address, see _resolve_pii_path).
      2. Read the field CSV (SampleID -> SiteID — no PII is read from here).
      3. Compose: SampleID -> SiteID -> Address.
 
    Either source missing => empty mapping (caller fills "Unknown Address").
    """
    base_dir = os.path.dirname(os.path.abspath(__file__))
 
    pii_path = _resolve_pii_path()
    if not pii_path or not os.path.exists(pii_path):
        return {}
 
    try:
        pii_df = pd.read_csv(pii_path, dtype=str).fillna("")
        pii_df.columns = pii_df.columns.str.strip()
        if "SiteID" not in pii_df.columns or "Address" not in pii_df.columns:
            return {}
        site_to_addr = dict(zip(
            pii_df["SiteID"].astype(str).str.strip(),
            pii_df["Address"].astype(str).str.strip(),
        ))
        site_to_addr = {k: v for k, v in site_to_addr.items() if k and v}
    except Exception:
        return {}
 
    # Field CSV — written by src/field_entry.py. New format: a plain
    # single-header CSV with just SiteID + SampleID. (The old file
    # `XRF Site Analysis Database W SampleID(Sheet1).csv` had a
    # two-row header and many extra columns — replaced for clarity and
    # to avoid PII drift.)
    ...
    # SiteID is only filled on the first row of each block — forward-fill so
    # every SampleID inherits its block's SiteID.
    ...
    sample_to_addr: dict[str, str] = {}
    for _, row in field_df.iterrows():
        sid = str(row.get("SiteID", "")).strip()
        sample = str(row.get("SampleID", "")).strip()
        addr = site_to_addr.get(sid)
        if sample and addr:
            sample_to_addr[sample] = addr
    return sample_to_addr
...
def load_master_data():
    ...
    mapping = _load_site_address_mapping()
    if mapping:
        df['Site_Address'] = df['SampleID'].map(mapping).fillna("Unknown Address")
    else:
        df['Site_Address'] = "Unknown Address"
