...
if data_mode == "Site_Master_Data":
    manual_file_path = os.path.join("Data", "Site_Lab_Data_Manual", "Site_Lab_Data.csv")
    download_label = "Download Site Lab Data"
    download_filename = "Site_Lab_Data.csv"
else:
        ...
        if not is_stable:
            ...
            for idx, val in enumerate(readings):
                if data_mode == "Clinic_Master_Data":
                    ...
                    # Site Format — XRFID uses no "Manual_" prefix so it
                    # matches the auto-generated format in XRF_Soil_chem_v*.csv
                    # for downstream reconciliation.
                    new_records.append({
                        "SampleID": sample_id,
                        "XRFID": f"{date_str}-{idx+1}",
                        "LeadPPM": val,
                        "LeadAvg": current_avg if idx == 0 else "" 
                    })
            ...
            # --- TRIGGER RECONCILIATION (Site mode only) ---
            # The reconciler compares the latest XRF_Soil_chem_v*.csv against
            # Site_Lab_Data.csv, marks rows match/discrepancy/pending, and
            # refreshes XRF_Master_Data_KEY.csv when both sides agree.
            # Only runs in Site mode — Clinic data isn't part of the key flow.
            if data_mode == "Site_Master_Data":
                try:
                    import sys
