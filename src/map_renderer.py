...
# # # # ═══════════════════════════════════════════════════════════════
# # # #  OPTIONAL EXPORT OFFSETS FROM SITE BUILDER PREVIEW
# # # # ═══════════════════════════════════════════════════════════════
# # # def _export_offset_for_yard(site_config, yard_key):
# # #     """Return manual preview offset for a yard, if site_builder attached one.

# # #     This is intentionally private and optional. Normal saved configs do not need
# # #     it. It lets the download buttons render the exact preview position even
# # #     before the user permanently saves/bakes the anchor into site_configs.json.
# # #     """
# # #     offsets = site_config.get("_preview_export_offsets") or {}
# # #     if not offsets:
# # #         return {"e": 0.0, "n": 0.0, "r": None}
# # #     key = yard_key or "legacy"
# # #     raw = offsets.get(key) or offsets.get("legacy") or {}
# # #     return {
# # #         "e": float(raw.get("e", raw.get("offset_east_ft", 0.0)) or 0.0),
# # #         "n": float(raw.get("n", raw.get("offset_north_ft", 0.0)) or 0.0),
# # #         # r is an ABSOLUTE rotation copied from the preview controls.
# # #         "r": None if raw.get("r", raw.get("rotation_deg", None)) is None else float(raw.get("r", raw.get("rotation_deg", 0.0)) or 0.0),
# # #     }

# # # def _shift_latlon_by_feet(lat, lon, east_ft, north_ft):
# # #     """Shift a lat/lon by east/north feet using the same approximation as Leaflet preview."""
# # #     earth_radius_ft = 20925721.78
# # #     lat = float(lat or 0.0)
# # #     lon = float(lon or 0.0)
# # #     dlat = (float(north_ft or 0.0) / earth_radius_ft) * (180.0 / math.pi)
# # #     dlon = (float(east_ft or 0.0) / (earth_radius_ft * math.cos(math.radians(lat)))) * (180.0 / math.pi)
...
# # #     # Strip heavy fields before embedding in JS payload. Include the
# # #     # per-block anchor + rotation so JS can project each block correctly.
# # #     # If site_builder attached _preview_export_offsets, apply those offsets
# # #     # HERE so downloaded HTML opens at the exact preview-dragged location.
# # #     blocks_js = []
# # #     for b in blocks:
# # #         off = _export_offset_for_yard(site_config, b.get("yard"))
# # #         shifted_lat, shifted_lon = _shift_latlon_by_feet(
# # #             b["anchor_lat"], b["anchor_lon"], off["e"], off["n"]
# # #         )
# # #         blocks_js.append({
# # #             "id": b["id"],
# # #             "cell_id": b.get("cell_id", b["id"]),
...
# # #             "corners": b["corners"],
# # #             "cx": b["cx"], "cy": b["cy"],
# # #             "anchor_lat": shifted_lat,
# # #             "anchor_lon": shifted_lon,
# # #             "rotation_deg": b["rotation_deg"] if off["r"] is None else off["r"],
# # #             "ppm": b["ppm"] if b["ppm"] is not None else 0,
# # #             "color": b["color"],
# # #             "label": b["label"],
# # #             "hasData": b["has_real_data"],
...
# # #     points_js = []
# # #     for p in points:
# # #         off = _export_offset_for_yard(site_config, p.get("yard"))
# # #         shifted_lat, shifted_lon = _shift_latlon_by_feet(
# # #             p["anchor_lat"], p["anchor_lon"], off["e"], off["n"]
# # #         )
# # #         points_js.append({
# # #             "id": p["id"],
# # #             "name": p.get("name", p["id"]),
...
# # #             "anchor_lat": shifted_lat,
# # #             "anchor_lon": shifted_lon,
# # #             "rotation_deg": p["rotation_deg"] if off["r"] is None else off["r"],
# # #             "ppm": p["ppm"] if p["ppm"] is not None else 0,
# # #             "color": p["color"],
# # #             "label": p["label"],
# # #             "hasData": p["has_real_data"],
...
# # #     for b in blocks:
# # #         # Apply THIS block's rotation (from its own yard) to its corners.
# # #         # If site_builder attached _preview_export_offsets, also apply the
# # #         # preview East/North offset in feet so the PNG matches the preview.
# # #         off = _export_offset_for_yard(site_config, b.get("yard"))
# # #         block_rot = b["rotation_deg"] if off["r"] is None else off["r"]
# # #         rotated_corners = []
# # #         for c in b["corners"]:
# # #             x_r, y_r = rotate_point(c[0], c[1], block_rot)
# # #             rotated_corners.append((x_r + off["e"], y_r + off["n"]))
...
# # #     for p in points:
# # #         off = _export_offset_for_yard(site_config, p.get("yard"))
# # #         pt_rot = p["rotation_deg"] if off["r"] is None else off["r"]
# # #         ox_r, oy_r = rotate_point(p["ox"], p["oy"], pt_rot)
# # #         ox_r += off["e"]; oy_r += off["n"]
# # #         ax.plot(
# # #             ox_r, oy_r, marker="o", markersize=11,
# # #             color=p["color"], markeredgecolor=edge_c, markeredgewidth=1.5,
# # #         )
...
# #   cursor:pointer;font-size:10px}}
# # .controls button:hover{{background:rgba(78,205,196,0.25)}}
# # .controls input{{width:100%;box-sizing:border-box;margin-top:6px;padding:5px 7px;
# #   border-radius:4px;border:1px solid rgba(255,255,255,0.16);
# #   background:rgba(255,255,255,0.08);color:#e8eaed;font-size:10px}}
# # .controls .small-status{{font-size:9px;color:#7a8599;margin-top:4px;min-height:12px}}
# # .controls .apply-btn{{width:100%;font-weight:bold;background:rgba(46,204,113,0.14);color:#7bed9f}}
# # .rotate-row{{display:flex;gap:4px;align-items:center;margin-top:4px}}
# # .rotate-row button{{margin:0;padding:2px 6px;font-size:10px}}
# # .rotate-row span{{font-size:10px;color:#7a8599;min-width:30px;text-align:center}}
# # .toggle-btn{{position:fixed;top:55px;right:20px;z-index:1002;padding:6px 10px;
...
# #   </div>
# #   <div class="offset" id="od">Offset: 0.0 E, 0.0 N</div>
# #   <input id="offsetPaste" placeholder="Paste: East, North, Rotation" title="Example: -1.43, 5.56, 0 or East=-1.43, North=5.56, Rot=0°">
# #   <button class="apply-btn" onclick="applyPastedOffset()">Apply Pasted Offset</button>
# #   <div class="small-status" id="pasteStatus">After downloading, paste copied preview offset here.</div>
# #   <button onclick="rs()">Reset</button>
# #   <button onclick="co()">Copy Current Position</button>
# #   <button onclick="tc()">Hide Panel</button>
# # </div>
# # <div class="legend"><b>Lead Guidelines (ppm)</b><br>{legend_rows}</div>
# # <script>
# # var SN={sn_flag};
# # var SITE_ID={json.dumps(site_id)};
# # var BL={json.dumps(blocks_js)};
# # var PT={json.dumps(points_js)};
# # var ANCHORS={json.dumps(anchor_markers)};
# # var RF=20925721.78;
...
# # function rg(d){{rot+=d;dg();}}
# # function rs(){{
# #   oE=0;oN=0;rot=LEGACY_ROT;
# #   try{{localStorage.removeItem('soil_map_offset_'+String(SITE_ID));}}catch(e){{}}
# #   dg();
# #   var st=document.getElementById('pasteStatus');
# #   if(st) st.textContent='Reset to downloaded default.';
# # }}
# # function co(){{
# #   var t=oE.toFixed(2)+', '+oN.toFixed(2)+', '+rot.toFixed(2);
# #   navigator.clipboard.writeText(t).then(function(){{alert('Copied: '+t);}});
# # }}
# # function parseOffsetText(txt){{
# #   txt = String(txt || '').trim();
# #   if(!txt) return null;
# #   // Accept both formats:
# #   //   -1.43, 5.56, 0
# #   //   East=-1.43, North=5.56, Rot=0°
# #   var nums = txt.match(/[-+]?\d*\.?\d+(?:[eE][-+]?\d+)?/g);
# #   if(!nums || nums.length < 2) return null;
# #   return {{
# #     e: parseFloat(nums[0]),
# #     n: parseFloat(nums[1]),
# #     r: nums.length >= 3 ? parseFloat(nums[2]) : rot
# #   }};
# # }}
# # function applyOffsetValues(vals, saveIt){{
# #   if(!vals || !isFinite(vals.e) || !isFinite(vals.n) || !isFinite(vals.r)) return false;
# #   oE = vals.e;
# #   oN = vals.n;
# #   rot = vals.r;
# #   dg();
# #   if(saveIt){{
# #     try{{localStorage.setItem('soil_map_offset_'+String(SITE_ID), JSON.stringify(vals));}}catch(e){{}}
# #   }}
# #   var st=document.getElementById('pasteStatus');
# #   if(st) st.textContent='Applied: '+oE.toFixed(2)+' E, '+oN.toFixed(2)+' N, '+rot.toFixed(2)+'°';
# #   return true;
# # }}
# # function applyPastedOffset(){{
# #   var inp=document.getElementById('offsetPaste');
# #   var vals=parseOffsetText(inp ? inp.value : '');
# #   var ok=applyOffsetValues(vals, true);
# #   if(!ok){{
# #     var st=document.getElementById('pasteStatus');
# #     if(st) st.textContent='Could not read offset. Use: East, North, Rotation';
# #   }}
# # }}
...
# # var pasteInput=document.getElementById('offsetPaste');
# # if(pasteInput){{
# #   pasteInput.addEventListener('paste', function(){{
# #     setTimeout(function(){{ applyPastedOffset(); }}, 50);
# #   }});
# #   pasteInput.addEventListener('keydown', function(ev){{
# #     if(ev.key === 'Enter') applyPastedOffset();
# #   }});
# # }}
# # try{{
# #   var saved=localStorage.getItem('soil_map_offset_'+String(SITE_ID));
# #   if(saved){{ applyOffsetValues(JSON.parse(saved), false); }}
# #   else {{ dg(); }}
# # }}catch(e){{ dg(); }}
# # </script>
# # </body></html>
...
# .legend b{{font-size:13px}}
# .legend .drag-handle{{display:inline-block;color:#7a8599;font-size:10px;
#   margin-left:6px;font-weight:normal;cursor:move}}
# /* Push Leaflet zoom control below the title bar and make it horizontal */
# .leaflet-top.leaflet-left{{top:55px}}
# .leaflet-control-zoom{{display:flex !important;flex-direction:row !important;
#   box-shadow:0 1px 5px rgba(0,0,0,0.4)}}
# .leaflet-control-zoom a{{border-radius:0 !important;
#   border-right:1px solid rgba(0,0,0,0.2) !important;
#   border-bottom:none !important}}
# .leaflet-control-zoom a:first-child{{border-radius:4px 0 0 4px !important}}
# .leaflet-control-zoom a:last-child{{border-radius:0 4px 4px 0 !important;
#   border-right:none !important}}
# /* Anchor toggle button — sits at the bottom-right, below the control panel */
# .anchor-toggle{{position:fixed;bottom:30px;right:20px;z-index:1002;
#   padding:6px 10px;border-radius:8px;
#   border:1px solid rgba(255,255,255,0.15);
#   background:rgba(12,15,20,0.9);color:#ffd166;cursor:pointer;
#   font-size:11px;font-weight:bold;transition:opacity 0.3s}}
# .anchor-toggle.off{{color:#7a8599}}
# .controls{{position:fixed;top:50px;right:20px;z-index:1001;
#   background:rgba(12,15,20,0.93);padding:12px 16px;border-radius:10px;
#   color:#e8eaed;font-size:11px;border:1px solid rgba(255,255,255,0.08);
#   min-width:200px;transition:transform 0.3s,opacity 0.3s}}
...
# <span class="meta">{title_meta}</span></div>
# <div id="map"></div>
# <button id="anchorTb" class="anchor-toggle" onclick="toggleAnchors()" title="Show/hide fixed point markers">📍 Anchors: ON</button>
# <button id="tb" class="toggle-btn" onclick="tc()" style="opacity:0;pointer-events:none">⚙</button>
# <div id="cp" class="controls">
#   <div class="yard-panel" id="front-panel">
#     <b>Front Yard Position</b>
#     <div class="hint">Click & drag a front yard block on the map</div>
#     <div class="rotate-row">
#       <button onclick="rg('front',-5)">−5°</button>
#       <button onclick="rg('front',-1)">−1°</button>
#       <span id="rd_front">0°</span>
#       <button onclick="rg('front',1)">+1°</button>
#       <button onclick="rg('front',5)">+5°</button>
#     </div>
#     <div class="offset" id="od_front">Offset: 0.0 E, 0.0 N</div>
#     <input id="paste_front" placeholder="Paste: East, North, Rotation" title="Example: -1.43, 5.56, 0 or East=-1.43, North=5.56, Rot=0°">
#     <button class="apply-btn" onclick="applyPastedOffset('front')">Apply Pasted Offset</button>
#     <div class="small-status" id="pasteStatus_front">Paste/apply offset for front yard only.</div>
#     <button onclick="copyOffset('front')">Copy Current Front Position</button>
#     <button onclick="resetYard('front')">Reset Front Yard</button>
#   </div>

#   <div class="yard-panel" id="back-panel" style="margin-top:14px;padding-top:12px;border-top:1px solid rgba(255,255,255,0.12);">
#     <b>Back Yard Position</b>
#     <div class="hint">Click & drag a back yard block on the map</div>
#     <div class="rotate-row">
#       <button onclick="rg('back',-5)">−5°</button>
#       <button onclick="rg('back',-1)">−1°</button>
#       <span id="rd_back">0°</span>
#       <button onclick="rg('back',1)">+1°</button>
#       <button onclick="rg('back',5)">+5°</button>
#     </div>
#     <div class="offset" id="od_back">Offset: 0.0 E, 0.0 N</div>
#     <input id="paste_back" placeholder="Paste: East, North, Rotation" title="Example: -1.43, 5.56, 0 or East=-1.43, North=5.56, Rot=0°">
#     <button class="apply-btn" onclick="applyPastedOffset('back')">Apply Pasted Offset</button>
#     <div class="small-status" id="pasteStatus_back">Paste/apply offset for back yard only.</div>
#     <button onclick="copyOffset('back')">Copy Current Back Position</button>
#     <button onclick="resetYard('back')">Reset Back Yard</button>
#   </div>

#   <button onclick="tc()" style="margin-top:12px;width:100%;">Hide Panel</button>
# </div>
# <div id="legendBox" class="legend"><b>Lead Guidelines (ppm)</b><span class="drag-handle">⋮⋮ drag</span><br>{legend_rows}</div>
# <script>
# var SN={sn_flag};
# var SITE_ID={json.dumps(site_id)};
# var BL={json.dumps(blocks_js)};
...
# // Plot each anchor (one per yard, or one for legacy) — stored so we can toggle visibility
# var ANCHOR_MARKERS=[];
...
#     }})
#   }}).addTo(map).bindTooltip(a.label);
#   ANCHOR_MARKERS.push(m);
# }});

# function toggleAnchors(){{
#   anchorsVisible=!anchorsVisible;
#   var btn=document.getElementById('anchorTb');
#   ANCHOR_MARKERS.forEach(function(m){{
#     if(anchorsVisible){{ m.addTo(map); }}
#     else {{ map.removeLayer(m); }}
#   }});
#   if(btn){{
#     btn.textContent = anchorsVisible ? '📍 Anchors: ON' : '📍 Anchors: OFF';
#     btn.classList.toggle('off', !anchorsVisible);
#   }}
# }}
...
# // Per-yard drag-offset overlay. Front/back are independent.
# var yardState = {{
#   front: {{ e: 0, n: 0, r: 0 }},
#   back:  {{ e: 0, n: 0, r: 0 }}
# }};
...
# function normYard(y){{
#   y = String(y || 'front').toLowerCase();
#   return (y === 'back' || y === 'backyard') ? 'back' : 'front';
# }}
# function yardStorageKey(y){{
#   return 'soil_map_offset_'+String(SITE_ID)+'_'+normYard(y);
# }}
# function hasYard(y){{
#   y = normYard(y);
#   return BL.some(function(b){{ return normYard(b.yard) === y; }}) ||
#          PT.some(function(p){{ return normYard(p.yard) === y; }});
# }}
# function updatePanel(y){{
#   y = normYard(y);
#   if(!yardState[y]) return;
#   var ys = yardState[y];
#   var od = document.getElementById('od_' + y);
#   var rd = document.getElementById('rd_' + y);
#   if(od){{
#     od.textContent = 'Offset: ' + ys.e.toFixed(1) + ' E, ' + ys.n.toFixed(1) + ' N' +
#       (ys.r ? (' | ' + ys.r.toFixed(1) + '°') : '');
#   }}
#   if(rd){{ rd.textContent = ys.r.toFixed(1) + '°'; }}
# }}
# function persistYardState(y){{
#   y = normYard(y);
#   try{{ localStorage.setItem(yardStorageKey(y), JSON.stringify(yardState[y])); }}catch(err){{}}
# }}
# function loadYardState(y){{
#   y = normYard(y);
#   try{{
#     var raw = localStorage.getItem(yardStorageKey(y));
#     if(raw){{
#       var parsed = JSON.parse(raw);
#       yardState[y] = {{
#         e: parseFloat(parsed.e || 0) || 0,
#         n: parseFloat(parsed.n || 0) || 0,
#         r: parseFloat(parsed.r || 0) || 0
#       }};
#     }}
#   }}catch(err){{}}
# }}
# function setStatus(y, msg){{
#   y = normYard(y);
#   var st = document.getElementById('pasteStatus_' + y);
...
#   BL.forEach(function(b){{
#     var y = normYard(b.yard);
#     var ys = yardState[y] || {{ e:0, n:0, r:0 }};
#     var blockRot = (b.rotation_deg || 0) + (ys.r || 0);

#     var ll=b.corners.map(function(c){{
#       var r=rp(c[0],c[1],blockRot);
#       return f2ll(b.anchor_lat,b.anchor_lon,r[1]+ys.n,r[0]+ys.e);
#     }});
#     var pl=L.polygon(ll,{{
#       color:'white',weight:2,fillColor:b.color,fillOpacity:0.75
#     }});
...
#     var rc=rp(b.cx,b.cy,blockRot);
#     var lp=f2ll(b.anchor_lat,b.anchor_lon,rc[1]+ys.n,rc[0]+ys.e);
#     var lh;
#     if(SN&&b.hasData){{
#       lh='<div style="font-family:Arial;text-align:center;line-height:1.1;pointer-events:none">'+
#          '<b style="font-size:10px;color:white;text-shadow:0 1px 3px rgba(0,0,0,0.8)">'+displayId+'</b><br>'+
...
#   PT.forEach(function(p){{
#     var y = normYard(p.yard);
#     var ys = yardState[y] || {{ e:0, n:0, r:0 }};
#     var ptRot = (p.rotation_deg || 0) + (ys.r || 0);
#     var r=rp(p.ox,p.oy,ptRot);
#     var ll=f2ll(p.anchor_lat,p.anchor_lon,r[1]+ys.n,r[0]+ys.e);
#     var cm=L.circleMarker(ll,{{
#       radius:8,color:'white',weight:2,
#       fillColor:p.color,fillOpacity:0.85
#     }});
...
#   dg();
# }}
# function resetYard(y){{
#   y = normYard(y);
#   if(!yardState[y]) return;
#   yardState[y] = {{ e:0, n:0, r:0 }};
#   try{{ localStorage.removeItem(yardStorageKey(y)); }}catch(err){{}}
#   dg();
#   setStatus(y, 'Reset '+y+' yard to downloaded default.');
# }}
# function copyOffset(y){{
#   y = normYard(y);
#   if(!yardState[y]) return;
#   var ys = yardState[y];
#   var t = ys.e.toFixed(2)+', '+ys.n.toFixed(2)+', '+ys.r.toFixed(2);
#   navigator.clipboard.writeText(t).then(function(){{ alert('Copied '+y+': '+t); }});
# }}
# function parseOffsetText(txt, fallbackRot){{
#   txt = String(txt || '').trim();
#   if(!txt) return null;
#   // Accept both formats:
#   //   -1.43, 5.56, 0
...
#     e: parseFloat(nums[0]),
#     n: parseFloat(nums[1]),
#     r: nums.length >= 3 ? parseFloat(nums[2]) : fallbackRot
#   }};
# }}
# function applyPastedOffset(y){{
#   y = normYard(y);
#   var inp=document.getElementById('paste_' + y);
#   var vals=parseOffsetText(inp ? inp.value : '', yardState[y].r);
#   if(!vals || !isFinite(vals.e) || !isFinite(vals.n) || !isFinite(vals.r)){{
#     setStatus(y, 'Could not read offset. Use: East, North, Rotation');
#     return;
#   }}
#   yardState[y] = {{ e: vals.e, n: vals.n, r: vals.r }};
#   persistYardState(y);
#   dg();
#   setStatus(y, 'Applied '+vals.e.toFixed(2)+' E, '+vals.n.toFixed(2)+' N, '+vals.r.toFixed(2)+'°');
# }}

# var iD=false, activeYard=null, dL=null, dE=0, dN=0;
# map.on('mousedown',function(e){{
#   var foundYard = null;
#   gl.eachLayer(function(l){{
#     if(l instanceof L.Polygon && l.getBounds().contains(e.latlng)){{
#       foundYard = l._yardKey || 'front';
#     }}
#   }});
#   if(foundYard){{
#     activeYard = normYard(foundYard);
#     iD=true;
#     dL=e.latlng;
#     dE=yardState[activeYard].e;
#     dN=yardState[activeYard].n;
#     map.dragging.disable();
#     map.getContainer().style.cursor='grabbing';
#   }}
# }});
# map.on('mousemove',function(e){{
#   if(!iD || !activeYard) return;
#   var refLat = CENTER_LAT;
#   yardState[activeYard].n = dN + (e.latlng.lat-dL.lat)*(Math.PI/180)*RF;
#   yardState[activeYard].e = dE + (e.latlng.lng-dL.lng)*(Math.PI/180)*RF*Math.cos(refLat*Math.PI/180);
#   persistYardState(activeYard);
#   dg();
# }});
# map.on('mouseup',function(){{
#   if(iD){{
...
#   var inp = document.getElementById('paste_' + y);
#   if(inp){{
#     inp.addEventListener('paste', function(){{ setTimeout(function(){{ applyPastedOffset(y); }}, 50); }});
#     inp.addEventListener('keydown', function(ev){{ if(ev.key === 'Enter') applyPastedOffset(y); }});
#   }}
# }});
# loadYardState('front');
# loadYardState('back');
# if(!hasYard('front')){{
#   var fp = document.getElementById('front-panel');
#   if(fp) fp.style.display = 'none';
# }}
# if(!hasYard('back')){{
#   var bp = document.getElementById('back-panel');
#   if(bp) bp.style.display = 'none';
# }}

# // ─── Make the Lead Guidelines legend box draggable ───
# (function(){{
...
#   try{{
#     var saved=localStorage.getItem('soil_legend_pos_'+SITE_ID);
#     if(saved){{
#       var p=JSON.parse(saved);
#       if(isFinite(p.left) && isFinite(p.top)){{
#         legend.style.left=p.left+'px';
#         legend.style.top=p.top+'px';
#         legend.style.bottom='auto';
#       }}
#     }}
#   }}catch(err){{}}
#   legend.addEventListener('mousedown',function(e){{
#     // Don't start drag from interactive children if any are added later
#     dragging=true;
...
#     if(!dragging) return;
#     var nx=startLeft+(e.clientX-sx);
#     var ny=startTop+(e.clientY-sy);
#     // Keep within viewport
#     nx=Math.max(0, Math.min(window.innerWidth-legend.offsetWidth, nx));
#     ny=Math.max(0, Math.min(window.innerHeight-legend.offsetHeight, ny));
...
#     dragging=false;
#     try{{
#       localStorage.setItem('soil_legend_pos_'+SITE_ID,
#         JSON.stringify({{ left: parseFloat(legend.style.left)||0,
#                           top:  parseFloat(legend.style.top)||0 }}));
#     }}catch(err){{}}
#   }});
#   // Touch support
...
#     var nx=startLeft+(e.touches[0].clientX-sx);
#     var ny=startTop+(e.touches[0].clientY-sy);
#     nx=Math.max(0, Math.min(window.innerWidth-legend.offsetWidth, nx));
#     ny=Math.max(0, Math.min(window.innerHeight-legend.offsetHeight, ny));
#     legend.style.left=nx+'px';
#     legend.style.top=ny+'px';
...
#                        show_numbers=True, use_mock_fallback=False,
#                        light_theme=True):
#     """Render a PNG map (no basemap) with optional value labels.

#     By default uses a light/white theme suitable for printed handouts
#     and embedding in resident PPTX reports. Pass `light_theme=False`
#     to restore the legacy dark-themed export (#1a1c24 background).

#     Style summary (light theme):
#       - White background, dark edges around cells
#       - Colored cells with dark pill behind cell IDs (high contrast)
#       - PPM values underneath each ID when show_numbers=True
#       - Legend in bottom-right with light chrome
...
#     coordinate space — that's expected when both yards measure from
#     nearby fixed points. Each block is colored by its own zone, so the
#     yards are still visually distinguishable.
#     """
#     site_id = site_config.get("site_id", "Site")
...
#     # Theme colors — light_theme=True is the new default for printable
#     # output. light_theme=False keeps the legacy dark export available.
#     if light_theme:
#         bg = "white"
#         text_c = "#222222"           # title + axis text
#         edge_c = "#2a2d38"           # cell borders + point marker rings
#         point_label_c = "#444444"    # italic point-sample name text
#         legend_face = "#f5f5f7"      # soft off-white legend background
#         legend_edge = "#cccccc"
#         legend_label = "#222222"
#     else:
#         bg = "#1a1c24"
...
#             sx, sy = min(xs), min(ys)
#             w, h = max(xs) - sx, max(ys) - sy
#             # Plain rectangle — no rounded-corner padding (which caused
#             # overlapping edges between adjacent cells) and no alpha
#             # blending (which made shared borders appear doubled).
#             patch = Rectangle(
#                 (sx, sy), w, h,
#                 facecolor=b["color"], edgecolor=edge_c,
...
#         display_name = p.get("name", p["id"])
#         ax.text(ox_r, oy_r + 2.5, display_name, ha="center", va="bottom",
#                  fontsize=8, color=point_label_c, fontstyle="italic")
#         if show_numbers and p["has_real_data"] and p["ppm"] is not None:
#             ax.text(ox_r, oy_r - 2.5, f"{p['ppm']:.0f}",
#                      ha="center", va="top",
#                      fontsize=7, color="white", fontweight="bold")
...
#     # Anchor markers intentionally omitted from PNG output —
#     # the fixed-point triangle and label cluttered cells near origin
#     # and aren't needed for the printable/PPTX-embedded view.
...
#     ax.set_aspect("equal")
#     # Hide axis scale ticks/labels and frame for a clean export
#     ax.set_xlabel("")
#     ax.set_ylabel("")
#     ax.set_xticks([])
#     ax.set_yticks([])
...
#     ax.legend(
#         handles=legend_patches, loc="lower right", fontsize=7,
#         framealpha=0.92, facecolor=legend_face, edgecolor=legend_edge,
...
function dg(){{
  ...
  BL.forEach(function(b){{
    ...
    var lh;
    if(SN&&b.hasData){{
      // Headline: PPM number — bold, larger, on a dark pill for emphasis.
      // Sub-label: cell ID — plain weight, smaller, sits below the number.
      lh='<div style="font-family:Arial;text-align:center;line-height:1.15;pointer-events:none">'+
         '<span style="display:inline-block;font-size:11px;font-weight:bold;'+
         'color:white;background:rgba(0,0,0,0.55);padding:1px 6px;border-radius:6px;">'+
         Math.round(b.ppm)+'</span><br>'+
         '<span style="font-size:9px;color:white;font-weight:normal;'+
         'text-shadow:0 1px 2px rgba(0,0,0,0.7)">'+displayId+'</span></div>';
    }} else {{
      // No ppm to show — cell ID stays bold + pill so it remains legible.
      lh='<div style="font-family:Arial;text-align:center;pointer-events:none">'+
         '<b style="font-size:10px;color:white;text-shadow:0 1px 3px rgba(0,0,0,0.8)">'+displayId+'</b></div>';
    }}
    gl.addLayer(L.marker(lp,{{
      icon:L.divIcon({{className:'',html:lh,iconSize:[60,36],iconAnchor:[30,18]}}),
      interactive:false
    }}));
  }});
...
def render_static_png(site_config, master_df, output_path,
    ...
    for b in blocks:
        ...
        # Label hierarchy:
        #   - When we have a real ppm value AND show_numbers is on, the
        #     PPM number is the headline (bold, white text on dark pill)
        #     and the cell ID sits below it as plain, non-bold text.
        #   - Otherwise (no data, or numbers hidden), only the cell ID is
        #     shown — and it gets the pill so it stays readable on any
        #     colored cell.
        display_id = b.get("cell_id", b["id"])

        if show_numbers and b["has_real_data"] and b["ppm"] is not None:
            # PPM number — highlighted + bold, centered.
            ax.text(
                cx_r, cy_r, f"{b['ppm']:.0f}",
                ha="center", va="center",
                fontsize=11, fontweight="bold", color="white",
                bbox=dict(boxstyle="round,pad=0.25",
                          facecolor="black", alpha=0.55, edgecolor="none"),
            )
            # Cell ID — plain, non-bold, sits below the highlighted number.
            ax.text(
                cx_r, cy_r - cell_h * 0.30, display_id,
                ha="center", va="center",
                fontsize=9, color="white", alpha=0.95,
            )
        else:
            # No ppm to show — cell ID alone, keeps the pill for legibility.
            ax.text(
                cx_r, cy_r, display_id,
                ha="center", va="center",
                fontsize=10, fontweight="bold", color="white",
    ...
    for p in points:
        ...
        display_name = p.get("name", p["id"])
        if show_numbers and p["has_real_data"] and p["ppm"] is not None:
            # PPM above the dot — bold, on dark pill.
            ax.text(
                ox_r, oy_r + 2.8, f"{p['ppm']:.0f}",
                ha="center", va="bottom",
                fontsize=9, fontweight="bold", color="white",
                bbox=dict(boxstyle="round,pad=0.2",
                          facecolor="black", alpha=0.55, edgecolor="none"),
            )
            # Sample name below the dot — plain, non-bold.
            ax.text(
                ox_r, oy_r - 2.8, display_name,
                ha="center", va="top",
                fontsize=8, color=point_label_c, fontstyle="italic",
            )
        else:
            # No ppm to show — keep the name as the only label, above the dot.
            ax.text(
                ox_r, oy_r + 2.5, display_name,
                ha="center", va="bottom",
                fontsize=8, color=point_label_c, fontstyle="italic",
            )
        all_x.append(ox_r); all_y.append(oy_r)
